...
c$$$        x1    = xsdisc(i)
c$$$        x2    = ysdisc(i)
c$$$        call diagquad(ier,eps,npoly,x1,x2,nquad,xs,ys,whts)
...
c!$OMP   DO
        ...
        stop
c        xsall(1:nquad,i+ndisc)   = xs(1:nquad)
c        ysall(1:nquad,i+ndisc)   = ys(1:nquad)
cc        whtsall(1:nquad,i+ndisc) = whts(1:nquad)
c        nquadall(i+ndisc)        = nquad
        end do
c!$OMP   END DO
c!$OMP   END PARALLEL
c
...
c$$$        write(iw,0100) "double precision :: xsall(300,:)"
c$$$        write(iw,0100) "double precision :: ysall(300,:)"
c$$$        write(iw,0100) "double precision :: whtsall(300,:)"
c$$$        write(iw,0100) "integer          :: nquadsall(:)"
c$$$c
c$$$
c$$$        do i=1,ndisc+nquadbdy
c$$$        nq = nquadall(i)
...
c$$$        call flush(iw)
c$$$c
c$$$c        Construct the second set of singular rules
c$$$c
c$$$        allocate(xs(10000),ys(10000),whts(10000))
c$$$
c$$$        x1 = 0.1d0
c$$$        x2 = 0.0d0
c$$$        call diagquad2(ier,eps,npoly,x1,x2,nquad,xs,ys,whts)
c$$$        stop
c$$$
c$$$        nn = ndisc+nquadbdy
c$$$c
...
c$$$        xsall(1:nquad,i+ndisc)   = xs(1:nquad)
c$$$        ysall(1:nquad,i+ndisc)   = ys(1:nquad)
c$$$        whtsall(1:nquad,i+ndisc) = whts(1:nquad)
c$$$        nquadall(i+ndisc)        = nquad
c$$$        end do
c$$$!$OMP   END DO
c$$$!$OMP   END PARALLEL
c$$$c
c$$$c       Write the second set of singular rules to the disc
c$$$c
c$$$        iw = 1001
c$$$        open(iw,FILE='quads.f90',STATUS='OLD',ACCESS='append')
c$$$
c$$$        write(iw,0100) "subroutine singquads2(nquadsall,xsall," //
c$$$     -    "ysall,whtsall)"
c$$$        write(iw,0100) "implicit double precision (a-h,o-z)"
c$$$        write(iw,0100) "double precision :: xsall(300,:)"
c$$$        write(iw,0100) "double precision :: ysall(300,:)"
...
c$$$        call flush(iw)
c$$$c
c$$$ 2100 format(A," = ",I3.3)
c$$$c
c$$$        iw = 1001
c$$$        open(iw,FILE='quads.f90',STATUS='OLD',ACCESS='append')
c$$$
c$$$        write(iw,0100) "subroutine discquad_info(nquad,nquadbdy," //
c$$$     -    "npoly)"
c$$$        
c$$$        write(iw,0100) "implicit double precision (a-h,o-z)"
c$$$        write(iw,2100) "nquad",ndisc
c$$$        write(iw,2100) "nquadbdy",nquadbdy
c$$$        write(iw,2100) "npoly",npoly
c$$$        write(iw,0100) "end subroutine"
c$$$        write(iw,*)    ""
c$$$        write(iw,*)    ""
c$$$        close(iw)
...
c
        ...
        verts(2,3) =  1
c
        call adaptri(ier,eps0,verts,funquad4,x1,x2,i1,j1,
     1    val1,nquad01)
c
        verts(1,1) = -1
        verts(2,1) = -1
...
c
        ...
        verts(2,3) =  1
c
        call adaptri(ier,eps0,verts,funquad5,x1,x2,i1,j1,
     1    val1,nquad01)
c
        verts(1,1) = -1
        verts(2,1) = -1
...
c
        ...
        double precision              :: xs(1),ys(1),whts(1)
        double precision              :: verts(2,3)
        double precision              :: xslege(100),whtslege(100)
        integer, allocatable          :: idxs(:)
...
c
     ...
     -    funuser3,npoly,x1,x2,eps,disc,ldisc,ncoefs,lkeep)
        if (ier .ne. 0) then
        call prinf("after legedisc, ier = *",ier,1)
        stop
        endif
...
c
        call legegs(eps,disc,nfuns3,funuser3,npoly,x1,x2,eps,
     1    krank,coefs)
        call prinf("after legegs, krank=*",krank,1)
c
c       Fetch the initial, oversampled, quadrature rule.
...
c
        ...
        ifadap    = 1
c
        call legedisc(ier,ifadap,nints0,ab0,kdisc,eps,nfuns4,
...
c
c       Add in a quadrature around (x1,x2)
c
        ...
        d = x2+1.0d-4
c
        print *,sum(whts(1:nquad))


        do i=1,npoly
        do j=1,npoly
        nquad       = nquad + 1
        xs(nquad)   = xslege(i)*(b-a)/2 + (b+a)/2
        ys(nquad)   = xslege(j)*(d-c)/2 + (d+c)/2
        whts(nquad) = whtslege(i)*whtslege(j)*(b-a)/2*(d-c)/2
        end do
        end do
c
        print *,sum(whts(1:nquad))
...
c$$$        verts(2,3) =  1
c$$$c
c$$$        call adaptri(ier,eps,verts,funquad6,x1,x2,i1,j1,
c$$$     1    val1,nquad01)
c$$$c
c$$$        verts(1,1) = -1
c$$$        verts(2,1) = -1
...
c$$$        y   = ys(i)
c$$$        wht = whts(i)
c$$$        call funquad6(x,y,x1,x2,i1,j1,val00)
c$$$        val = val + wht*val00
c$$$        end do
c$$$c
c$$$        idx = idx + 1
...
c$$$        verts(2,3) =  1
c$$$c
c$$$        call adaptri(ier,eps,verts,funquad7,x1,x2,i1,j1,
c$$$     1    val1,nquad01)
c$$$c
c$$$        verts(1,1) = -1
c$$$        verts(2,1) = -1
...
c$$$        y   = ys(i)
c$$$        wht = whts(i)
c$$$        call funquad7(x,y,x1,x2,i1,j1,val00)
c$$$        val = val + wht*val00
c$$$        end do
c$$$c
c$$$        idx = idx + 1
...
c
        ...
        subroutine funuser3(y2,vals,npoly,x1,x2,eps)
        implicit double precision (a-h,o-z)
        double precision vals(1),pols(0:100)
...
c$$$        do ideg1=0,ndeg
c$$$        ideg2 = ndeg-ideg1
c$$$        call adapgauss(ier,a1,b1,eps,m,val1,funquad2,x1,x2,y2,
c$$$     1    ideg1,par5,par6,par7,par8)
c$$$        call adapgauss(ier,a2,b2,eps,m,val2,funquad2,x1,x2,y2,
c$$$     1    ideg1,par5,par6,par7,par8)
c$$$        val = val1+val2
c$$$        idx       = idx + 1
...
c$$$        do ideg1=0,ndeg
c$$$        ideg2 = ndeg-ideg1
c$$$        call adapgauss(ier,a1,b1,eps,m,val1,funquad3,x1,x2,y2,
c$$$     1    ideg1,par5,par6,par7,par8)
c$$$        call adapgauss(ier,a2,b2,eps,m,val2,funquad3,x1,x2,y2,
c$$$     1    ideg1,par5,par6,par7,par8)
c$$$        val = val1+val2
c$$$        idx       = idx + 1
c$$$        vals(idx) = val*pols(ideg2)
