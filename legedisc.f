...
c
        ...
        call legedisc(ier,ngrid,xsgrid,k,epsdisc,nfuns,funuser,nfuns,
...
c
     ...
     1    par4,krank,coefs)
c
c       Construct a quadrature for the functions.
c
c$$$        call legequad(disc,nquad0,xs0,whts0)
c$$$        call chebquad(krank,funeval,disc,krank,coefs,par4,
c$$$     1    nquad0,xs0,whts0,nquad,xs,whts,rints)
c$$$c
c$$$        ngoal=-1
c$$$        ifaccept=0
c$$$c
c$$$        call gaussquad(epsnewt,krank,rints,funeval,disc,krank,coefs,
c$$$     1    par4,nquad,xs,whts,a,b,ngoal,ifaccept)
c$$$c
...
c
c   legegs2 - performs the pivoted Gram-Schmidt algorihtm on a 
c       collection of user-supplied functions.  A random transform
c       is used to reduce the memory requirements of this routine;
c       this allows for tremedously large collections of input 
c       functions.
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc


c$$$        subroutine legegs2(eps,disc,nfuns,krankmax,funuser,par1,
c$$$     -    par2,par3,par4,krank,coefs)
c$$$        implicit double precision (a-h,o-z)
c$$$        dimension disc(1),coefs(1)
c$$$        integer, allocatable :: ipivs(:)
c$$$        external funuser
c$$$c
c$$$c       Gram-Schmidt a collection of user-supplied input functions for
c$$$c       which a discretization has been produced.  This subroutine
c$$$c       returns coefficient expansions representing the resulting
c$$$c       orthonormal basis.
c$$$c
c$$$c       This version of the routine uses a randomized transform
c$$$c       which allows it to handle very large collections of functions.
c$$$c
c$$$c       IMPORTANT WARNING: the calling syntax for funuser is different 
c$$$c       for this function than for legegs.
c$$$c
c$$$c       The coefs array must be of length 
c$$$c
c$$$c           max( (krankmax)*nfuns, ncoefs*krankmax )
c$$$c
c$$$c                            Input Parameters:
c$$$c
c$$$c   eps - precision for the GS procedure
c$$$c   disc - a structure describing the discretization of the input 
c$$$c       functions, presumably produced by legedisc
c$$$c   nfuns - the number of user-supplied input functions
c$$$c
c$$$c   funuser - an external subroutine supplying the values of the
c$$$c       input functions at a point
c$$$c
c$$$c       funuser(x,i,val,par1,par2,par3,par4)
c$$$c
c$$$c       Return the value of the ith user-supplied function at the point
c$$$c       x.
c$$$c
c$$$c   
c$$$c
...
c$$$c
c$$$c   krank - the dimension of the orthonormal basis spanning the input
c$$$c       functions
c$$$c   coefs - a (ncoefs,krank) each column of which contains the
c$$$c       coefficient expansion for one function in the orthonormal
c$$$c       basis generated by the GS procedure
c$$$c
c$$$
c$$$c
c$$$c       Fetch data from the disc structure.
c$$$c
c$$$        k     = disc(1) 
c$$$        nints = disc(2)
c$$$        a     = disc(3) 
...
c$$$        ncoefs = k*nints
c$$$c
c$$$c       Call an auxillary subroutine to shape arrays.
c$$$c
c$$$        allocate(ipivs(nfuns+krankmax+10))
c$$$c
c$$$        call legegs20(eps,nints,k,disc(iab),disc(ixs),disc(iwhts),
c$$$     -    krankmax,ncoefs,coefs,krank,nfuns,ipivs,
c$$$     -    funuser,par1,par2,par3,par4)
c$$$c
c$$$        call legegs21(eps,nints,k,disc(iab),disc(ixs),disc(iwhts),
c$$$     -    disc(iu),krank,ipivs,coefs,funuser,par1,par2,par3,par4)
c$$$c
c$$$        end
c$$$c
c$$$
c$$$
c$$$        subroutine legegs21(eps,nints,nquad,ab,xs,whts,u,krank,ipivs,
c$$$     -    coefs,funuser,par1,par2,par3,par4)
c$$$        implicit double precision (a-h,o-z)
c$$$        dimension xs(1),whts(1),ipivs(1),u(nquad,nquad),ab(2,1)
c$$$        dimension coefs(nquad,nints,krank)
c$$$        external funuser
c$$$c
c$$$        double precision, allocatable :: rnorms(:),xx(:)
c$$$        allocate(rnorms(krank+nints*nquad+10))
c$$$c
c$$$        ncoefs = nints*nquad
c$$$c
c$$$c       Copy the selected functions in to the coefs matrix.
c$$$c        
c$$$        do 1000 i=1,krank
c$$$        ifun = ipivs(i)
c$$$        do 1100 int=1,nints
c$$$        a = ab(1,int)
c$$$        b = ab(2,int)
c$$$        alpha = (b-a)/2
c$$$        beta  = (b+a)/2
...
c$$$        x   = alpha*xs(j)+beta
c$$$        wht = alpha*whts(j)
c$$$        call funuser(x,ifun,val,par1,par2,par3,par4)
c$$$        coefs(j,int,i)=val*sqrt(wht)
c$$$ 1200 continue
c$$$ 1100 continue
c$$$ 1000 continue
c$$$c
c$$$c       Gram-Schmidt the matrix.
c$$$c
c$$$        call legepiv2(coefs,ncoefs,krank,rnorms)
c$$$        call prin2("in legegs2, rnorms = *",rnorms,krank)
c$$$c
c$$$c       Convert the values of those functions to coefficients.
c$$$c
c$$$        allocate(xx(nquad))
c$$$c
c$$$        do 3000 ifun=1,krank
c$$$        do 3100 int=1,nints
c$$$        a = ab(1,int)
c$$$        b = ab(2,int)
c$$$        alpha = 1/sqrt((b-a)/2)
c$$$        do 3200 i=1,nquad
c$$$        sum=0
c$$$        do 3300 j=1,nquad
c$$$        sum=sum+u(i,j)*coefs(j,int,ifun)*alpha
c$$$ 3300 continue
c$$$        xx(i) = sum
c$$$ 3200 continue
c$$$c
c$$$        do 3400 j=1,nquad
c$$$        coefs(j,int,ifun)=xx(j)
c$$$ 3400 continue
c$$$c
c$$$ 3100 continue
c$$$ 3000 continue
...
c$$$
c$$$
c$$$        subroutine legegs20(eps,nints,nquad,ab,xs,whts,krankmax,ncoefs,
c$$$     -    coefs,krank,nfuns,ipivs,fununser,par1,par2,par3,par4)
c$$$        implicit double precision (a-h,o-z)
c$$$        dimension ab(2,1),xs(1),whts(1),coefs(krankmax,nfuns)
c$$$        dimension ipivs(1)
c$$$c
c$$$        double precision, allocatable :: w(:),xx(:,:),yy(:)
c$$$        double precision, allocatable :: rnorms(:)
c$$$c
c$$$        n      = ncoefs
c$$$        nsteps = 7
c$$$        lw     = 1 000 000
c$$$        allocate(w(lw))
c$$$        call random_transf_init(nsteps,n,w,lkeep)
c$$$c
c$$$        call prinf("after random_transf_init, n= * ",n,1)
c$$$c
c$$$c       Apply the random transform.
c$$$c
c$$$        allocate(xx(nquad,nints),yy(n))
c$$$c
c$$$        do 1000 i=1,nfuns
c$$$        do 1100 int=1,nints
c$$$        a = ab(1,int)
...
c$$$        x   = alpha*xs(j)+beta
c$$$        wht = alpha*whts(j)
c$$$        call fununser(x,i,val,par1,par2,par3,par4)
c$$$        xx(j,int) = val*sqrt(wht)
c$$$ 1200 continue
c$$$ 1100 continue
c$$$c
c$$$        call random_transf(xx,yy,w)
c$$$c
c$$$        do 1300 l=1,krankmax
c$$$        coefs(l,i) = yy(l)
c$$$ 1300 continue
c$$$ 1000 continue
c$$$c
c$$$c       Gram-Schmidt the transformed matrix.
c$$$c
c$$$        allocate(rnorms(nfuns+krankmax+10))
c$$$c
c$$$        call legepiv(coefs,krankmax,nfuns,eps,rnorms,ipivs,krank)
c$$$c
c$$$        call prinf("in legegs2, krank = *",krank,1)
c$$$        call prin2("in legegs2, rnorms = *",rnorms,krank)
c$$$c
c$$$        end
...
c
c
        subroutine legedisc(ier,ngrid,xs0,k,eps,nfuns,
     -    funuser,par1,par2,par3,par4,disc,ldisc,ncoefs,lkeep)
        implicit double precision (a-h,o-z)
        dimension disc(1),xs0(1)
        external funuser
...
c                          Input Parameters:
c
c   ngrid - the number of points in the initial discretization grid
c   xs0 - a list of the endpoints of the intervals in the initial discretization
!       grid
c   k - the number of nodes used in the piecewise expansions on each
c       interval
c   eps - precision for the discretizations
...
c       Construct the subintervals.
c
        call discfuns(ier,eps,ngrid,xs0,nfuns,funuser,par1,par2,par3,
     -     par4,maxints,disc(iab),nints,k,disc(ixs),disc(iwhts),
     -     disc(iu))
c
...
c
c
        subroutine discfuns(ier,eps,ngrid,xs0,nfuns,
     -     funuser,par1,par2,par3,  par4,maxints,ab,nints,k,xs,whts,u)
        implicit double precision (a-h,o-z)
        dimension ab(2,maxints),xs(1),whts(1),u(k,k),xs0(1)
c        dimension stack(2,100 000),vals(k,nfuns),vals0(nfuns+100)
...
c$$$        val=a(i)
c$$$        j=i-1
c$$$        do 1100 while (j .ge. 1 .AND. a(j) .gt. val) 
c$$$        a(j+1)=a(j)
c$$$        j=j-1
c$$$ 1100 continue
c$$$        a(j+1)=val
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c       Legendre and Chebyshev evaluation routines.
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c$$$c
...
c$$$        dimension vals(1)
c$$$c     
c$$$c       Evaluate the Legendre polynomials of degree 0 through n
c$$$c       at the point x.
c$$$c
c$$$        vals(1) = 1
c$$$        if (n .eq. 0) return
...
c$$$c
c$$$c
c$$$        subroutine cheb(n,x,vals)
c$$$        implicit double precision (a-h,o-z)
c$$$        dimension vals(1)
c$$$c     
c$$$c       Evaluate the Chebyshev polynomials of degree 0 through n
c$$$c       at the point x.
c$$$c
c$$$        vals(1) = 1
c$$$        if (n .eq. 0) return
...
c$$$c
c$$$c
c$$$        subroutine chebders(n,x,vals,ders)
c$$$        implicit double precision (a-h,o-z)
c$$$        dimension vals(1),ders(1)
c$$$c     
c$$$c       Evaluate the Chebyshev polynomials of degree 0 through n
c$$$c       and their derivatives at the point x.
c$$$c
c$$$        vals(1) = 1
c$$$        ders(1) = 0
c$$$        if (n .eq. 0) return
...
c$$$        do 1000 j=2,n
c$$$        vals(j+1)=d*vals(j)-vals(j-1)
c$$$        ders(j+1)=2*vals(j)+d*ders(j)-ders(j-1)
c$$$ 1000 continue
c$$$c
c$$$        end
c$$$c
